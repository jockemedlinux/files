...
####################################################################################
echo -e "####################################################################################"
echo -e '\e[93mRun script this way for logging "./backup-script.sh | tee `date +%F`.log". \e[0m'
echo -e "####################################################################################\n"
...
if [[ $answer == "Y" || $answer == "y" ]]; then
    ...
    tar czvf --ignore-failed-read --warning=no-file-changed "$dockerbackupdir/$(date +%F)/docker-directories.tar.gz" /srv/*
...
if [[ $cloudanswer == "Y" || $cloudanswer == "y" ]]; then
   ...
   tar czvf --ignore-failed-read --warning=no-file-changed "$cloudbackupdir/$(date +%F)/nextcloud.tar.gz" /mnt/DATADISK/_CLOUD/*
